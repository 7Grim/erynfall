-- ============================================================================
-- OSRS-MMORP Complete Database Schema for SQL Server 2025 (CORRECTED)
-- ============================================================================
-- FIXES: Removed duplicate PRIMARY KEY constraints, fixed syntax errors
-- Created: 2026-03-21 (FIXED VERSION)
...
CREATE TABLE osrs.items (
  ...
  CHECK (tradeable IN (0, 1)),
  CHECK (weight_kg >= 0),
  CHECK (high_alchemy_value >= 0 AND low_alchemy_value >= 0)
);
GO
...
CREATE TABLE osrs.npcs (
  ...
  size INT DEFAULT 1 CHECK (size IN (1, 2, 3, 4)),
  attackable BIT DEFAULT 1,
  aggressive BIT DEFAULT 0,
  description VARCHAR(500),
  created_at DATETIME2 DEFAULT GETDATE(),
  
  CHECK (attack_level >= 1 AND strength_level >= 1 AND defence_level >= 1)
);
GO
...
CREATE TABLE osrs.npc_loot_tables (
  ...
  drop_rate INT NOT NULL CHECK (drop_rate > 0 AND drop_rate <= 10000),
...
CREATE TABLE osrs.players (
  ...
  CHECK (LEN(username) >= 3 AND LEN(username) <= 12)
);
GO
...
CREATE TABLE osrs.ge_orders (
  ...
  INDEX idx_ge_matching ON osrs.ge_orders (item_id, is_buy, price_per_unit DESC, created_at, filled_quantity) WHERE completed_at IS NULL,
  INDEX idx_ge_player (player_id, completed_at),
  INDEX idx_ge_created (created_at),
  INDEX idx_ge_item (item_id)
);
GO
...
CREATE TABLE osrs.chat_messages (
  ...
  chat_type INT DEFAULT 0 CHECK (chat_type IN (0, 1)),
...
BEGIN
  ...
  BEGIN
    RAISERROR('XP delta out of bounds', 16, 1);
    RETURN;
  END
  ...
  BEGIN
    UPDATE osrs.players 
    SET attack_xp = CASE WHEN attack_xp + @xp_delta > @max_xp THEN @max_xp ELSE attack_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'STRENGTH'
  BEGIN
    UPDATE osrs.players 
    SET strength_xp = CASE WHEN strength_xp + @xp_delta > @max_xp THEN @max_xp ELSE strength_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'DEFENCE'
  BEGIN
    UPDATE osrs.players 
    SET defence_xp = CASE WHEN defence_xp + @xp_delta > @max_xp THEN @max_xp ELSE defence_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'MAGIC'
  BEGIN
    UPDATE osrs.players 
    SET magic_xp = CASE WHEN magic_xp + @xp_delta > @max_xp THEN @max_xp ELSE magic_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'PRAYER'
  BEGIN
    UPDATE osrs.players 
    SET prayer_xp = CASE WHEN prayer_xp + @xp_delta > @max_xp THEN @max_xp ELSE prayer_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'WOODCUTTING'
  BEGIN
    UPDATE osrs.players 
    SET woodcutting_xp = CASE WHEN woodcutting_xp + @xp_delta > @max_xp THEN @max_xp ELSE woodcutting_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'FISHING'
  BEGIN
    UPDATE osrs.players 
    SET fishing_xp = CASE WHEN fishing_xp + @xp_delta > @max_xp THEN @max_xp ELSE fishing_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE IF @skill_name = 'COOKING'
  BEGIN
    UPDATE osrs.players 
    SET cooking_xp = CASE WHEN cooking_xp + @xp_delta > @max_xp THEN @max_xp ELSE cooking_xp + @xp_delta END
    WHERE id = @player_id;
  END
  ELSE
  BEGIN
    RAISERROR('Invalid skill name', 16, 1);
    RETURN;
  END
END
GO
...
BEGIN
  ...
  DECLARE @idle_seconds INT = 1800;
  DECLARE @old_packet_seconds INT = 300;
...
BEGIN
  ...
  DECLARE @objective_bitmask INT = (1 << @objective_index);
